...
def convert_to_conductance_continuous(weights, Gmax, Gmin, Tmin):
    """
    Continuous (no quant) mapping: w→G→T.
    weights: np.ndarray of floats (can be any real range)
    """
    abs_w = np.abs(weights)
    Wmax  = abs_w.max() if abs_w.max() != 0 else 1.0

    # 1) normalize into [0,1]
    w_norm    = abs_w / Wmax

    # 2) linear map into device conductance window
    G_levels  = Gmin + w_norm * (Gmax - Gmin)

    # 3) thickness from conductance
    T_fil     = Tox + (Tox - Tref) * np.log(G_levels / alpha)

    # 4) branch assignments
    T_pos     = np.where(weights >= 0, T_fil, Tmin)
    T_neg     = np.where(weights  < 0, T_fil, Tmin)
...
def load_and_process_images(image_path, size, vmax, model, binarize):
    """
    1) Loads all .png/.jpg images from `image_path`
    2) If binarize=True: Otsu‐threshold → {0,1} → {0,Vmax}
       else: map [0–255]→[0–Vmax] linearly
    3) Dumps ./tmp/img_to_voltage_data.csv for Spectre, and
    4) Returns (binary_images_dict, voltage_data_dict)
    """
    voltage_data   = {}
    binary_images  = {}  # will stay empty if binarize=False

    for fname in sorted(os.listdir(image_path), key=natural_sort_key):
        if not fname.lower().endswith(('.png','jpg','jpeg')):
            continue
        # — load & flatten —
        img = Image.open(os.path.join(image_path, fname)).convert('L').resize(size)
        arr = np.array(img).flatten().astype('float32')
        norm = arr / 255.0

        if binarize:
            thresh = threshold_otsu(norm)
            bin_img   = np.where(norm > thresh, 1, 0)
            voltages  = (bin_img * vmax).astype('float32')
            binary_images[fname] = bin_img.reshape(size)
            # optional inference on binarized input
            if model is not None:
                pred = model.predict(norm.reshape(1, size[0], size[1]))
                print(f"[BINARY] {fname} → {np.argmax(pred,axis=1)}")
        else:
            voltages = np.round(norm * vmax, 2).astype('float32')

        voltage_data[fname] = voltages

    # write out the CSV that SpiceCim.py will consume
    df = pd.DataFrame.from_dict(voltage_data, orient='index')
    os.makedirs('tmp', exist_ok=True)
    df.to_csv('tmp/img_to_voltage_data.csv', header=True, index_label='image_name')
    
    if model is not None:
        with open('tmp/tensorOut.csv', 'w', newline='') as tf:
            tf.write('image_name,software_pred\n')
            for fname in sorted(voltage_data.keys(), key=natural_sort_key):
                if binarize:
                    # Predict on the binarized image
                    input_arr = binary_images[fname].astype('float32')
                else:
                    # Predict on the original normalized image
                    img       = Image.open(os.path.join(image_path, fname)).convert('L').resize(size)
                    input_arr = np.array(img).astype('float32') / 255.0
                pred       = model.predict(input_arr.reshape(1, size[0], size[1]))
                pred_class = int(np.argmax(pred, axis=1)[0])
                tf.write(f'{fname},{pred_class}\n')

    # return two things, so callers doing “a, b = …” won’t break
    return binary_images, voltage_data
...
def map_secondary_input(input_path_pos, input_path_neg, full_path, sim_time):
    ...
    # Read the necessary source lines from _pwl_sources.scs
    with open('./tmp/_pwl_sources_ll.scs', 'r') as file:
        lines = file.readlines()
    selected_sources = "".join(lines[start_line:end_line])
...
def generate_pwl_sources(csv_path, pulse_width, trise, tfall, scol):
    print("Inside generate_pwl_sources")
    # Load CSV file
    ...
    # Write to file
    print("PWL signals are written to ./tmp/_pwl_sources.scs")
...
def generate_pwl_sources_ll(csv_path, pulse_width, trise, tfall, scol):
    # Load CSV file
    df = pd.read_csv(csv_path, index_col=0)  # Assuming the first column is image names and used as index
    ...
    # Write to file
    with open('./tmp/_pwl_sources_ll.scs', 'w') as file:
        file.write(content)
    total_rows = df.shape[0]
    return total_rows, time_start
